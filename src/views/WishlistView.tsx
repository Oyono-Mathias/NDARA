...
export function WishlistView() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(qWish, async (snap) => {
      ...
      try {
          // Firebase in limite is 10, so we might need chunking if courseIds > 10, but let's assume slice(0, 10).
          // For safety we chunk it or just do slice(0, 10) for UI preview.
          ...
          if (coursesData.length > 0) {
            const instructorIds = [...new Set(coursesData.map((c: any) => c.instructorId).filter(Boolean))];
  ...
  return (
    <div className="flex flex-col gap-8 pb-24 bg-black min-h-screen relative overflow-hidden -mt-32 max-w-md mx-auto z-10 w-full pt-32">
