...
export function CartView() {
  ...
  const handleCheckout = () => {
    ...
    navigate(`/student/checkout/${items[0].courseId}`);
  };
  ...
  return (
    <div className="flex flex-col gap-0 pb-40 min-h-screen relative overflow-hidden bg-slate-950 -mt-32 max-w-3xl mx-auto z-10 w-full pt-20">
      ...
      <main className="pt-6 px-6 space-y-8 relative z-10 animate-in fade-in duration-500">
        {isLoading ? (
            <div className="space-y-4">
                {[...Array(2)].map((_, i) => <div key={i} className="h-24 w-full rounded-[2rem] bg-slate-900 border border-white/5 animate-pulse" />)}
            </div>
        ) : items.length > 0 ? (
                    ...
                    {items.map(item => (
                        <div key={item.id} className="bg-[#1e293b] border border-white/5 rounded-[2rem] p-3 flex gap-4 shadow-xl active:scale-[0.98] transition-all">
                            <div className="relative h-20 w-24 shrink-0 rounded-[1.5rem] overflow-hidden bg-slate-800 shadow-inner">
                                <img src={item.imageUrl || `https://picsum.photos/seed/${item.id}/100/100`} alt={item.title} className="w-full h-full object-cover" />
                            </div>
                            <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-[#10b981] font-black text-base uppercase">{item.price?.toLocaleString('fr-FR')} <span className="text-[10px]">XAF</span></p>
                                    <button 
                ...
                <section className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button 
                            ...
                            className={`flex items-center justify-center gap-3 p-4 rounded-2xl border-2 transition-all active:scale-95
                        ...
                        <button 
                            ...
                            className={`flex items-center justify-center gap-3 p-4 rounded-2xl border-2 transition-all active:scale-95 opacity-50
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#1e293b]/95 backdrop-blur-xl border-t border-white/5 z-50 safe-area-pb shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div className="max-w-3xl mx-auto space-y-4 px-6 md:px-0">
                ...
                <button 
                    ...
                    className="w-full flex items-center justify-center h-16 rounded-[2rem] bg-[#10b981] hover:bg-[#10b981]/90 text-[#0f172a] font-black uppercase text-sm tracking-widest shadow-2xl shadow-[#10b981]/20 group transition-all active:scale-95"
                >
