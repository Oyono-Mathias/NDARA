...
export function InstructorCourseEdit() {
  ...
  if (error || !course) {
    return <div className="text-center text-red-500 py-20 font-bold">Impossible de charger le cours.</div>;
  }
  ...
  if (!isAuthorized) {
    return (
        <div className="max-w-2xl mx-auto py-20 px-4 text-center space-y-6">
            ...
            <Link to="/instructor/courses" className="inline-flex mt-8 h-14 rounded-2xl bg-primary hover:bg-primary/90 text-slate-950 font-black uppercase items-center text-xs tracking-widest px-10 transition-colors">
    ...
    <div className="space-y-8 bg-slate-900/50 p-6 -m-6 rounded-2xl min-h-full">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
                <Link to="/instructor/courses" className="flex items-center justify-center w-10 h-10 bg-[#1e293b] border border-white/10 rounded-xl text-white hover:bg-white/10 transition">
                    <ArrowLeft className="h-4 w-4" />
                </Link>
                <div>
                    <h1 className="text-2xl font-black text-white line-clamp-1 uppercase tracking-tight">{course.title}</h1>
                    <div className="flex items-center gap-2 mt-1">
                        <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-md border-none ${isPublished ? "bg-green-500/10 text-green-500" : isPending ? "bg-amber-500/10 text-amber-500" : "bg-slate-500/10 text-slate-400"}`}>
                            {isPublished ? 'Publié' : isPending ? 'En attente de validation' : 'Brouillon'}
                        </span>
                        {isRequestedBuyout && <span className="px-2 py-1 bg-primary/10 text-primary text-[9px] border-none font-black uppercase rounded-md">Rachat demandé</span>}
                        {isApprovedBuyout && <span className="px-2 py-1 bg-emerald-500/10 text-emerald-500 text-[9px] border-none font-black uppercase rounded-md">Acquis par Ndara</span>}
                    </div>
                </div>
            </div>
        ...
        <div className="w-full">
            <div className={`grid w-full bg-[#1e293b] p-1 rounded-xl h-auto gap-1 mb-6 ${!course.isPlatformOwned ? "grid-cols-3" : "grid-cols-2"}`}>
                <button onClick={() => setActiveTab('content')} className={`py-2.5 rounded-lg active:scale-95 transition-all font-bold uppercase text-[10px] tracking-widest ${activeTab === 'content' ? 'bg-primary text-slate-950 shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>Contenu</button>
                <button onClick={() => setActiveTab('details')} className={`py-2.5 rounded-lg active:scale-95 transition-all font-bold uppercase text-[10px] tracking-widest ${activeTab === 'details' ? 'bg-primary text-slate-950 shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>Détails</button>
                {!course.isPlatformOwned && (
                    <button onClick={() => setActiveTab('buyout')} className={`flex items-center justify-center py-2.5 rounded-lg active:scale-95 transition-all font-bold uppercase text-[10px] tracking-widest ${activeTab === 'buyout' ? 'bg-primary text-slate-950 shadow-md' : 'text-primary hover:bg-white/5'}`}>
