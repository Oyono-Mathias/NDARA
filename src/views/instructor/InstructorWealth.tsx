...
export function InstructorWealth() {
  ...
  const stats = useMemo(() => {
    ...
    const totalSalesEarned = payments.reduce(
      ...
      <main className="flex-1 overflow-y-auto pb-32 px-4 mt-6 space-y-8 animate-in fade-in duration-700">
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-[#10b981] text-[10px] font-black uppercase flex items-center gap-1">
              <span className="w-1.5 h-1.5 bg-[#10b981] rounded-full" /> Actif
            </span>
          </div>

          <TouchArea
            as="button"
            className="bg-gradient-to-br from-[#1e293b] to-slate-900 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative z-10 w-full text-left group hover:border-primary/50"
            onClick={() => setIsWithdrawModalOpen(true)}
          >
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-10">
                <div>
                  <p className="text-emerald-100 text-[10px] font-black uppercase tracking-[0.25em] mb-1">
                    Solde Disponible
                  </p>
                  <h2 className="text-white font-black text-4xl tracking-tight">
                    {stats.availableBalance.toLocaleString("fr-FR")}{" "}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-emerald-200 text-[9px] font-bold uppercase mb-1 tracking-wider">
                    Expert Titulaire
                  </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <TouchArea
              as="div"
              className="bg-slate-900 border border-white/5 rounded-3xl p-5 relative overflow-hidden"
            >
              ...
              <h4 className="text-white font-black text-2xl tracking-tight">
                {stats.primaryBalance.toLocaleString("fr-FR")}{" "}
                <span className="text-xs font-bold text-slate-500 uppercase">
                  F
                </span>
              </h4>
              <p className="text-slate-600 text-[9px] font-medium tracking-wide mt-1">
                Fonds débloqués & disponibles
              </p>
            </TouchArea>
            <TouchArea
              as="div"
              className="bg-slate-900 border border-white/5 rounded-3xl p-5 relative overflow-hidden"
            >
              <p className="text-[#10b981] text-[10px] font-black uppercase tracking-widest mb-1.5">
                Parrainages
              </p>
              <h4 className="text-[#10b981] font-black text-2xl tracking-tight">
                {stats.ambassadorAvailable.toLocaleString("fr-FR")}{" "}
                <span className="text-xs font-bold text-slate-500 uppercase">
                  F
                </span>
              </h4>
            ...
            <TouchArea
              ...
              className="bg-slate-900 border border-white/5 rounded-3xl p-5 relative overflow-hidden"
            >
              <p className="text-amber-500 text-[10px] font-black uppercase tracking-widest mb-1.5">
                En Séquestre
              </p>
              <h4 className="text-amber-400 font-black text-2xl tracking-tight">
                {stats.escrowBalance.toLocaleString("fr-FR")}{" "}
                <span className="text-xs font-bold text-slate-500 uppercase">
                  F
                </span>
              </h4>
              <p className="text-slate-600 text-[9px] font-medium tracking-wide mt-1">
                Libération sous 14 jours
              </p>
            </TouchArea>
          </div>
          ...
          <TouchArea
            as="button"
            onClick={() => setIsWithdrawModalOpen(true)}
            className="flex items-center justify-center w-full h-16 rounded-[2.5rem] bg-[#10b981] hover:bg-[#34d399] text-[#0f172a] font-black uppercase text-xs tracking-widest shadow-xl shadow-primary/20 border-none"
          >
        ...
        <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-black text-white text-[10px] uppercase tracking-[0.3em]">
              Journal des flux
            </h3>
          ...
          <div className="space-y-4">
            {loadingStates.payments || loadingStates.payouts ? (
              [...Array(3)].map((_, i) => (
                <Skeleton
                  key={i}
                  className="h-16 w-full rounded-2xl bg-[#1e293b]"
                />
              ))
              ...
              historyItems.map((item) => (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={`font-black text-sm mb-1 ${item.amount > 0 ? "text-[#10b981]" : "text-red-400"}`}
                    >
                      {item.amount > 0
                    ...
                    <span
                      className={`text-[8px] font-black uppercase border-none px-2 py-0.5 h-4 rounded-full ${
          ...
          <div className="bg-[#0f172a] border border-white/5 rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
            ...
            <div className="p-8 pb-4">
              ...
              <p className="text-gray-400 font-medium italic text-sm mt-2">
                Retirez vos gains vers votre compte personnel.
              </p>
            </div>
            <div className="p-8 space-y-6">
              <div className="bg-slate-950 rounded-3xl p-5 border border-white/5 text-center shadow-inner">
                ...
                <p className="text-3xl font-black text-[#10b981]">
                  {stats.availableBalance.toLocaleString("fr-FR")} FCFA
                </p>
              </div>
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <ProviderBtn
                    active={withdrawMethod === "orange"}
                    onClick={() => setWithdrawMethod("orange")}
                    label="Orange"
                    color="bg-orange-500"
                  />
                  <ProviderBtn
                    active={withdrawMethod === "mtn"}
                    onClick={() => setWithdrawMethod("mtn")}
                    label="MTN"
                    color="bg-yellow-500"
                  />
                  <ProviderBtn
                    active={withdrawMethod === "wave"}
            ...
            <div className="p-8 bg-slate-950/50 border-t border-white/5 flex gap-3 rounded-b-[2.5rem]">
              <button
                ...
                className="flex-1 h-14 rounded-2xl font-bold text-slate-500 hover:text-white hover:bg-white/5 transition uppercase text-[10px] tracking-widest"
              >
                Annuler
              </button>
              <button
...
function ProviderBtn({ active, onClick, label, color }: any) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all active:scale-95 gap-1 ${active ? "border-[#10b981] bg-[#10b981]/5" : "border-transparent bg-slate-950 grayscale opacity-50 hover:opacity-80"}`}
    >
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-black text-[10px] ${color}`}
      >
        {label.charAt(0)}
      </div>
...
function RevenueSkeleton() {
  return (
    <div className="p-6 space-y-8 min-h-screen bg-[#0f172a] -m-4 md:-m-8 md:p-8">
      <Skeleton className="h-10 w-1/2 bg-slate-800 rounded" />
      <Skeleton className="h-56 w-full rounded-[2.5rem] bg-slate-800" />
      <div className="space-y-4">
        <Skeleton className="h-24 w-full rounded-[2.5rem] bg-slate-800" />
        <Skeleton className="h-24 w-full rounded-[2.5rem] bg-slate-800" />
      </div>
    </div>
  );
