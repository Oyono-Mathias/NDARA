...
export function InstructorCoupons() {
  ...
  useEffect(() => {
     ...
     const qCourses = query(collection(db, 'courses'), where('instructorId', '==', currentUser.uid));
     const unsubCourses = onSnapshot(qCourses, snap => {
         setCourses(snap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
         setCoursesLoading(false);
     });

     const qCoupons = query(collection(db, 'course_coupons'), where('instructorId', '==', currentUser.uid), orderBy('createdAt', 'desc'));
      ...
      <main className="flex-1 space-y-6 animate-in fade-in duration-700">
        ...
        {couponsLoading ? (
            <div className="space-y-4">
                {[...Array(3)].map((_, i) => <div key={i} className="h-32 w-full rounded-[2rem] bg-slate-900 border border-white/5 animate-pulse" />)}
            </div>
        ) : (
