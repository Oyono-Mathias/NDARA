...
- finalGrade: La note finale sur 20 (sous forme de nombre)`;
  ...
  const pendingSubmissions = useMemo(
    () => submissions.filter((s) => s.status === "submitted" || !s.status),
    [submissions],
  );
        ...
        <div className="p-6 bg-[#1e293b] border border-white/5 rounded-[2rem] text-white shadow-xl animate-in fade-in">
          ...
          <div className="space-y-4 max-w-2xl">
            ...
            <button
              ...
              disabled={
                !selectedCourse || !newTitle || !newDescription || isCreating
              }
            ...
            <div className="mt-12">
              ...
              <div className="space-y-2">
                {assignments.map((a) => (
                  <div
                    key={a.id}
                    className="p-4 bg-slate-900 rounded-xl border border-white/5 flex justify-between items-center"
                  >
                    <div>
          ...
          {selectedSubmission ? (
            <div className="relative p-6 bg-[#1e293b] border border-white/5 rounded-[2rem] text-white shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              {gradeSuccess && (
                <div className="absolute inset-0 z-20 bg-emerald-500/10 backdrop-blur-sm rounded-[2rem] flex flex-col items-center justify-center animate-in fade-in zoom-in duration-300 border border-emerald-500/50">
                  ...
                  <p className="text-emerald-400 font-black tracking-widest uppercase text-lg">
                    Correction Publiée
                  </p>
                </div>
              )}
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <button
                  ...
                  className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center hover:bg-slate-700 transition tracking-tight shrink-0"
                >
                  <X size={18} />
                </button>
              </div>
                ...
                {selectedSubmission.submissionUrl && (
                  <div className="mt-4 pt-4 border-t border-slate-800">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center text-[10px] font-bold text-primary hover:text-emerald-400 transition-colors uppercase tracking-widest"
                    >
              ...
              <div className="space-y-4 relative z-10">
                <div className="flex justify-between items-center">
                  ...
                  >
                    ...
                    {isGeneratingAI
                      ? "Génération en cours..."
                      : "Générer une correction IA (Mathias)"}
                  </button>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-slate-400 uppercase text-[10px] tracking-widest border-b border-white/5 pb-2">
                ...
                {isLoadingSubmissions ? (
                  ...
                  <div className="space-y-3">
                    {pendingSubmissions.map((sub) => (
                      <SwipeableItem
                        key={sub.id}
                        onSwipeRight={() => setSelectedSubmission(sub)}
                        rightAction={
                      ...
                      >
                        <TouchArea
                          as="div"
                          className="p-4 bg-[#1e293b] rounded-2xl border border-primary/20 hover:border-primary/50 cursor-pointer transition animate-in slide-in-from-left-4"
                          onClick={() => setSelectedSubmission(sub)}
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="font-bold text-white text-sm">
                                {sub.assignmentTitle || sub.title}
                              </p>
                              <p className="text-xs text-slate-400 mt-1">
                                {sub.studentName || sub.studentId}
                              </p>
                            </div>
                            <span className="bg-primary/20 text-primary text-[10px] font-black uppercase px-2 py-1 rounded">
                              Nouveau
                            </span>
                          </div>
                        </TouchArea>
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-slate-400 uppercase text-[10px] tracking-widest border-b border-white/5 pb-2">
                  Historique Corrigé ({gradedSubmissions.length})
                </h3>
                <div className="space-y-3">
                  {gradedSubmissions.slice(0, 50).map((sub) => (
                    <div
                      key={sub.id}
                      className="p-4 bg-slate-900 rounded-2xl border border-white/5 opacity-70"
                    >
                      <div className="flex justify-between items-start">
                        <div>
                        ...
                        <div className="text-right flex items-center gap-2">
                          <span className="text-green-500 font-black bg-green-500/10 px-2 py-1 rounded-md">
                            {sub.grade}/20
                          </span>
                        </div>
                      </div>
