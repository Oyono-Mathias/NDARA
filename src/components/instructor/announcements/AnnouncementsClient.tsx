...
export function AnnouncementsClient() {
  ...
  return (
    <div className="space-y-8 mt-6 relative">
      ...
      <div className="bg-[#1e293b] border border-white/5 rounded-[2.5rem] p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        <p className="text-sm font-medium text-slate-400 mb-8 italic">
          Informez vos étudiants des nouveautés, des prochains live ou des mises
          à jour de contenu.
        </p>
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <TouchArea
              as="button"
              onClick={() => !isSubmitting && setIsCourseSheetOpen(true)}
              className="w-full bg-[#0f172a] border border-white/10 rounded-2xl h-14 px-4 text-sm text-white flex items-center justify-between"
            >
              <span
                className={!selectedCourse ? "text-slate-600" : "text-white"}
              >
                {selectedCourse
          ...
          <div className="space-y-2">
            ...
            <input
              className="w-full h-14 bg-[#0f172a] border border-white/10 rounded-2xl px-4 text-sm text-white focus:ring-1 focus:ring-orange-500/50 outline-none placeholder:text-slate-600"
          ...
          <div className="pt-2 flex justify-end">
            <TouchArea
              ...
              className="bg-orange-500 text-white h-14 px-8 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 hover:bg-orange-600 disabled:opacity-50 disabled:hover:bg-orange-500 transition shadow-lg shadow-orange-500/20 w-full md:w-auto"
            >
              {isSubmitting ? (
                <>
        ...
        {announcements.length === 0 ? (
          ...
          <div className="space-y-4">
            {announcements.map((announce) => (
              <div
                key={announce.id}
                className="p-6 bg-[#1e293b] border border-white/5 rounded-[2rem] group hover:border-white/10 transition relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-4">
                  <TouchArea
                    as="button"
                    onClick={() => handleDelete(announce.id)}
                    className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-500 hover:text-red-500 hover:bg-red-500/10 transition sm:opacity-0 sm:group-hover:opacity-100"
                ...
                <div className="pr-12">
                  <div className="flex items-center gap-3 mb-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-500 text-xs font-medium">
                      <Clock size={12} />
                      {announce.createdAt?.toDate
                        ? announce.createdAt.toDate().toLocaleString("fr-FR", {
                            day: "numeric",
                            month: "short",
                            hour: "2-digit",
                            minute: "2-digit",
        ...
        <div className="space-y-2 mt-4 pb-4">
          {courses.map((c) => (
            <TouchArea
              ...
              className={`w-full p-4 rounded-xl text-left font-medium flex items-center justify-between transition-colors ${selectedCourse === c.id ? "bg-orange-500/20 text-orange-400 border border-orange-500/30" : "bg-[#1e293b] text-white border border-transparent"}`}
            >
              <span>{c.title}</span>
              {selectedCourse === c.id && (
