  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/80 backdrop-blur-sm p-0 md:p-4 animate-in fade-in duration-300">
      <div className="w-full max-w-md p-8 bg-[#0f172a] border-t border-white/10 md:border md:rounded-[2.5rem] rounded-t-[2.5rem] shadow-2xl relative animate-in slide-in-from-bottom-8">
        <h2 className="font-black text-2xl text-white uppercase tracking-tight mb-2">
          Générer un Coupon
        </h2>
        <p className="text-slate-400 text-sm mb-8">
          Boostez vos ventes avec des codes promos limités.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-40 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
              {courses?.length === 0 ? (
                ...
                courses.map((c: any) => (
                  <label
                    key={c.id}
                    className="flex items-center gap-3 p-3 bg-slate-900 rounded-xl border border-white/5 cursor-pointer hover:border-primary/30 transition"
                  >
                    <input
                      type="checkbox"
            ...
            <p className="text-[10px] text-slate-500 italic px-2">
              Si aucun n'est sélectionné, s'appliquera à tous vos cours.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-8">
          <button
            onClick={onClose}
            className="flex-1 h-14 bg-slate-900 border border-white/5 text-slate-400 font-bold text-xs uppercase tracking-widest rounded-2xl hover:text-white transition"
          >
            Annuler
          </button>
          <button
