...
import { Paperclip, Plus, Trash2, File as FileIcon, ExternalLink, UploadCloud, Loader2, CheckCircle2 } from 'lucide-react';
...
export function ResourcesClient() {
    ...
    const handleAddResource = async () => {
        if (!selectedCourse || !resourceTitle || !resourceFile || !currentUser?.uid) return;
        ...
        try {
            ...
            const safeFileName = `${Date.now()}_${resourceFile.name.replace(/[^a-zA-Z0-9.\-]/g, '_')}`;
            const storageRef = ref(storage, `courses/${selectedCourse}/resources/${safeFileName}`);
            ...
            uploadTask.on('state_changed', 
                ...
                async () => {
                    // Upload complete, get URL and save to Firestore
    ...
    return (
        <div className="space-y-6 mt-6 relative">
            ...
            <div className="bg-[#1e293b] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                ...
                <p className="text-sm font-medium text-slate-400 mb-8 italic">Téléversez des documents (PDF, ZIP, DOCX) pour vos apprenants.</p>
                ...
                <div className="space-y-2 mb-6">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 ml-2">Fichier à téléverser</label>
                    <div className="flex gap-2 items-center">
                        ...
                        <button 
                            ...
                            disabled={!selectedCourse || !resourceTitle || !resourceFile || isUploading}
                            ...
                            <div key={res.id} className="p-4 bg-[#1e293b] border border-white/5 rounded-2xl flex items-start justify-between group hover:border-white/10 transition">
                                <div className="flex gap-4 items-start w-full pr-4">
                                    <div className="w-12 h-12 bg-slate-900 border border-white/5 rounded-xl flex items-center justify-center shrink-0 shadow-inner">
                                        ...
                                        ) : res.mimeType?.includes('zip') || res.mimeType?.includes('rar') ? (
                                    ...
                                    <div className="overflow-hidden flex-1">
                                        <p className="font-bold text-sm text-white mb-0.5 truncate">{res.title}</p>
                                        <div className="flex items-center gap-2 mb-2">
                                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest truncate">{res.courseTitle}</p>
                                            {res.fileSize && (
                                                <>
                                                    <span className="w-1 h-1 rounded-full bg-slate-600 shrink-0"></span>
                                                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest shrink-0">{formatBytes(res.fileSize)}</p>
                                                </>
                                            )}
                                        </div>
                                        <a href={res.url} target="_blank" rel="noreferrer" className="text-[11px] text-emerald-400 hover:text-emerald-300 hover:underline flex items-center gap-1 font-semibold">
