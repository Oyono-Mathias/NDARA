...
export function QuizPageClient() {
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      const quizRef = doc(
        collection(db, `courses/${selectedCourseId}/quizzes`),
      );
      ...
      {!isCreating ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {quizzes.length === 0 ? (
              ...
              quizzes.map((quiz) => (
                <div
                  key={quiz.id}
                  className="bg-[#1e293b] p-6 rounded-[2rem] border border-white/5 shadow-xl hover:border-primary/20 transition-all group cursor-pointer relative overflow-hidden"
                >
                  <div className="absolute top-0 left-0 w-full h-1 bg-primary/20 group-hover:bg-primary transition-colors" />
                  <h3 className="font-black text-white uppercase tracking-tight text-lg leading-tight mb-2">
                    {quiz.title}
                  </h3>
        ...
        <div className="bg-[#1e293b] border border-white/5 rounded-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom-4 relative">
          <button
            ...
            className="absolute top-6 right-6 w-10 h-10 rounded-full bg-slate-800 text-slate-400 flex items-center justify-center hover:text-white transition"
          >
            <X size={18} />
          </button>
          ...
          <div className="max-w-3xl space-y-8">
            ...
            <div className="pt-6 border-t border-white/5 space-y-8">
              <h3 className="font-bold text-lg text-white flex items-center gap-2">
                Questions{" "}
                <span className="bg-primary/10 text-primary text-[10px] uppercase font-black px-2 py-1 rounded">
                  {questions.length}
                </span>
              </h3>
              ...
              {questions.map((q, qIndex) => (
                <div
                  key={qIndex}
                  className="p-6 bg-slate-900 border border-slate-800 rounded-3xl relative"
                >
                  <span className="absolute -top-3 -left-3 w-8 h-8 rounded-full bg-primary flex items-center justify-center text-[#0f172a] font-black text-xs">
                    {qIndex + 1}
                  </span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 pt-2">
                      {q.options.map((opt, oIndex) => (
                        <div
                          key={oIndex}
                          className={`flex items-center gap-3 p-3 rounded-2xl border ${opt.isCorrect ? "border-primary/50 bg-primary/5" : "border-slate-800 bg-slate-950"}`}
                        >
                          <input
                            type="radio"
            ...
            <div className="pt-8 border-t border-white/5 flex justify-end gap-4">
              <button
                onClick={() => setIsCreating(false)}
                className="px-6 h-14 bg-slate-900 rounded-2xl text-slate-400 font-bold uppercase text-xs hover:text-white transition"
              >
                Annuler
              </button>
              <button
