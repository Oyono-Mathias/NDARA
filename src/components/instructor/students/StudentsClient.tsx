...
export function StudentsClient() {
  ...
  useEffect(() => {
    ...
    const unsubEnrollments = onSnapshot(qEnrollments, async (snap) => {
      const enrollList = snap.docs.map(
      ...
      const studentIds = Array.from(
        new Set(enrollList.map((e) => e.studentId)),
      ).filter(Boolean);
  ...
  const filteredStudents = useMemo(() => {
    return enrollments.filter((e) => {
      ...
      const studentName = (
        student.fullName || "Étudiant Anonyme"
      ).toLowerCase();
      ...
      // Course match
      const matchesCourse =
        selectedCourseId === "all" || e.courseId === selectedCourseId;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-[#1e293b] to-slate-900 border border-white/5 rounded-[2rem] p-6 shadow-xl relative overflow-hidden group">
          ...
          <p className="text-xs text-slate-400 mt-2 font-medium">
            Élèves ayant terminé leur syllabus
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#1e293b] border border-white/5 rounded-[2rem] p-6 space-y-6 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 h-12 px-6 rounded-2xl bg-[#0f172a]/45 border border-white/5 text-slate-300 font-bold uppercase text-xs tracking-wider md:w-auto w-full justify-center"
          >
      ...
      <div className="bg-[#1e293b]/40 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <h4 className="font-black text-slate-500 text-[10px] uppercase tracking-[0.3em]">
            Liste des Inscriptions ({filteredStudents.length})
          </h4>
          <span className="text-slate-500 text-[9px] font-mono flex items-center gap-1.5 uppercase">
            <Database size={10} /> Real-time
          </span>
        </div>
            ...
            filteredStudents.map((enrollment) => {
              ...
              return (
                <SwipeableItem
                  ...
                  onSwipeRight={() =>
                    navigate(
                      `/student/messages?newChatUser=${enrollment.studentId}`,
                    )
                  }
                  onSwipeLeft={() =>
                    console.log("Bannir", enrollment.studentId)
                  }
                  rightAction={
                ...
                >
                  <div className="bg-[#1e293b] p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-6 w-full">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-white text-base truncate tracking-tight">
                            {student.fullName || "Étudiant Ndara"}
                          </h4>
                          {hasFinished && (
                            <span
                              className="bg-emerald-500/10 text-emerald-400 rounded-md p-0.5"
                              title="Syllabus validé"
                            >
                              <CheckCircle size={14} />
                            </span>
                          )}
                    ...
                    <div className="flex-1 md:max-w-xs space-y-2">
                      <div className="flex justify-between items-center text-xs">
                        ...
                        <span
                          className={`font-black tracking-tight ${hasFinished ? "text-emerald-400" : "text-primary"}`}
                        >
                          {enrollment.progress}%
                        </span>
                      </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-black uppercase tracking-wider text-white">
              Ndara Coaching Mathias IA
            </h4>
            <p className="text-xs text-slate-400 leading-relaxed max-w-2xl font-medium">
              Glissez vers la droite sur un étudiant pour lui envoyer un message
              direct.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) =>
                  setSelectedProgressFilter(e.target.value as any)
                }
              >
