...
export function Sidebar({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
    ...
    const sidebarContent = (
        <div className="flex-1 overflow-y-auto hide-scrollbar pb-24">
            ...
            <div className="flex items-center justify-between p-6">
                ...
                <button onClick={onClose} className="md:hidden w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition">
                    <X size={20} />
                </button>
            </div>
            ...
            <div className="px-4 space-y-8">
                {isInstructorMode ? (
                    <>
                        ...
                        <div className="space-y-2">
                            <p className="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">• CONFIGURATION</p>
                            <NavItem icon={User} label="PROFIL EXPERT" to="/instructor/profile" current={location.pathname} onClick={onClose} />
                            <NavItem icon={Settings} label="RÉGLAGES" to="/instructor/settings" current={location.pathname} onClick={onClose} />
                        </div>
                    </>
                ) : (
                    <>
    ...
    return (
        <>
            {/* Mobile Drawer */}
            {isOpen && (
                <div className="fixed inset-0 z-[100] md:hidden flex justify-start transition-opacity duration-300">
